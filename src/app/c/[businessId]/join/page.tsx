...
import {
  Stamp, User, Phone, Mail, ArrowRight, Check, Smartphone,
  AlertCircle, Loader2
} from "lucide-react";
...
export default function ClientJoinPage() {
  ...
  const biz = mockBusiness; // In prod: fetch from Supabase by businessId
  ...
  const [cardId] = useState(() => `demo-${Date.now()}`);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError(null);
    // In prod: POST to /api/cards with { businessId, customerName, phone/email }
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="wallet-card p-5 mb-6 animate-float">
          ...
          {biz.mode === "stamps" ? (
            ...
            <>
              ...
              <p className="text-xs text-dark-400 mt-1.5">Prochain objectif : <strong className="text-white">{biz.goal} pts = {biz.reward_description}</strong></p>
            </>
          )}
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input value={name} onChange={e => setName(e.target.value)}
                    placeholder="Marie" className="input-field pl-10 text-sm" required />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={contactType === "phone" ? "tel" : "email"}
                    value={contact} onChange={e => setContact(e.target.value)}
                    placeholder={contactType === "phone" ? "514-555-0100" : "vous@exemple.com"}
                    className="input-field pl-10 text-sm" required />
                </div>
              </div>

              <button type="submit" disabled={loading || !name || !contact}
                className="btn-primary w-full flex items-center justify-center gap-2 py-3.5">
                {loading
                  ? <Loader2 size={18} className="animate-spin" />
                  : <><span>Créer ma carte</span><ArrowRight size={16} /></>}
              </button>
            </form>
          ...
          <div className="glass-card p-6 animate-slide-up">
            ...
            <div className="space-y-3">
              <button onClick={() => handleAddWallet("apple")} disabled={!!walletLoading}
                className="w-full flex items-center gap-4 p-4 bg-white text-dark-900 rounded-2xl font-semibold hover:bg-gray-100 transition-all active:scale-98 disabled:opacity-60">
              ...
              <button onClick={() => handleAddWallet("google")} disabled={!!walletLoading}
                className="w-full flex items-center gap-4 p-4 bg-dark-800 border border-white/10 text-white rounded-2xl font-semibold hover:bg-dark-700 transition-all active:scale-98 disabled:opacity-60">
        ...
        {step === "success" && (
          <div className="glass-card p-8 text-center animate-slide-up">
            ...
            <p className="text-xs text-dark-500">Vous recevrez des notifications push sur votre téléphone pour suivre votre progression.</p>
          </div>
        )}
      </div>
    </div>
