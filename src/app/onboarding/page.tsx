...
interface FormData {
  ...
  mode: Mode | null; goal: number; reward: string; pointsRate: number;
}
...
export default function OnboardingPage() {
  ...
  const [form, setForm] = useState<FormData>({ name: "", type: "", city: "", mode: null, goal: 10, reward: "", pointsRate: 10 });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const update = (key: keyof FormData, val: FormData[typeof key]) => setForm(f => ({ ...f, [key]: val }));
  ...
  const finish = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from("businesses").insert({
        ...
        type: form.type,
        city: form.city,
        mode: form.mode ?? "stamps",
        goal: form.goal,
        reward_description: form.reward,
        plan: "starter",
        points_rate: form.mode === "points" ? form.pointsRate : null,
      });
      ...
      <div className="relative w-full max-w-2xl glass-card p-8 animate-slide-up">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input value={form.name} onChange={e => update("name", e.target.value)}
                  placeholder="Ex : Salon Élite, Café Lumière..." className="input-field" />
              </div>
              <div>
                <label className="label">Type de commerce</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {businessTypes.map(({ value, label, icon: Icon }) => (
                    <button key={value} type="button" onClick={() => update("type", value)}
                      className={cn("p-3 rounded-xl border text-sm font-medium text-left flex items-center gap-2.5 transition-all",
                        form.type === value ? "bg-brand-500/20 border-brand-500/50 text-white shadow-glow" : "bg-dark-800/50 border-white/8 text-dark-300 hover:border-white/20 hover:text-white")}>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={form.city} onChange={e => update("city", e.target.value)} className="input-field appearance-none pr-10">
        ...
        {step === 2 && (
          <div>
            <div className="mb-8">
              ...
              <h1 className="text-2xl font-bold text-white">Quel type de programme ?</h1>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <button onClick={() => update("mode", "stamps")} className={cn("p-6 rounded-2xl border text-left transition-all",
                form.mode === "stamps" ? "bg-brand-500/15 border-brand-500/50 shadow-glow" : "bg-dark-800/50 border-white/8 hover:border-brand-500/30")}>
                <Stamp size={24} className={cn("mb-4", form.mode === "stamps" ? "text-brand-400" : "text-dark-400")} />
                <h3 className="font-bold text-white text-lg mb-1">Mode Tampons</h3>
                <p className="text-sm text-dark-400">1 visite = 1 tampon. Simple et universel.</p>
              ...
              <button onClick={() => update("mode", "points")} className={cn("p-6 rounded-2xl border text-left transition-all",
                form.mode === "points" ? "bg-cyan-500/10 border-cyan-500/50" : "bg-dark-800/50 border-white/8 hover:border-cyan-500/30")}>
                <TrendingUp size={24} className={cn("mb-4", form.mode === "points" ? "text-cyan-400" : "text-dark-400")} />
                <h3 className="font-bold text-white text-lg mb-1">Mode Points</h3>
                <p className="text-sm text-dark-400">1$ dépensé = points. Flexible.</p>
            ...
            <div className="space-y-5">
                ...
                <>
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <input type="range" min={5} max={20} value={form.goal}
                        onChange={e => update("goal", Number(e.target.value))} className="flex-1 accent-brand-500" />
                  ...
                  <div>
                    ...
                    <input value={form.reward} onChange={e => update("reward", e.target.value)}
                      placeholder="Ex : 1 coupe gratuite, café offert..." className="input-field" />
                    <div className="flex gap-2 mt-2">
                      {["1 coupe gratuite", "Café offert", "Lavage gratuit"].map(s => (
                        <button key={s} onClick={() => update("reward", s)}
                          className="text-xs px-3 py-1.5 bg-dark-800 border border-white/8 rounded-full text-dark-300 hover:text-white transition-all">
                          {s}
                        </button>
                      ))}
                    </div>
                ...
                <>
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[{ rate: 5, label: "1$ = 5 pts" }, { rate: 10, label: "1$ = 10 pts" }, { rate: 1, label: "1$ = 1 pt" }].map(({ rate, label }) => (
                        <button key={rate} onClick={() => update("pointsRate", rate)}
                  ...
                  <div className="flex gap-3">
                    <div className="flex-1">
                      <input type="number" placeholder="Ex : 500" value={form.goal || ""}
                        onChange={e => update("goal", Number(e.target.value))} className="input-field" min={1} />
                      <p className="text-xs text-dark-500 mt-1">Points requis</p>
                    </div>
                    <div className="flex-1">
                      <input value={form.reward} onChange={e => update("reward", e.target.value)}
                        placeholder="Ex : 5$ de rabais" className="input-field" />
              ...
              {form.reward && (
                <div className="p-4 glass border-brand-500/20 rounded-xl animate-slide-up">
                  <p className="text-xs text-brand-400 font-medium mb-2 flex items-center gap-1.5"><Sparkles size={12} /> Aperçu</p>
                  <p className="text-sm text-dark-200">"Félicitations ! Vous avez gagné : <strong className="text-white">{form.reward}</strong>."</p>
                </div>
              )}
            </div>
          </div>
        ...
        {step === 4 && (
          <div className="text-center py-8">
            ...
            <p className="text-dark-400 max-w-sm mx-auto mb-8">Votre QR code est généré. Accueillez votre premier client fidèle.</p>
            ...
            <button onClick={finish} disabled={loading} className="btn-primary inline-flex items-center gap-2 text-base py-3.5 px-8">
              {loading
                ...
                : <><span>Accéder à mon tableau de bord</span><ArrowRight size={18} /></>}
            </button>
          </div>
        )}
        ...
        {step < 4 && (
          <div className="flex justify-between items-center mt-8 pt-6 border-t border-white/5">
            {step > 1
              ? <button onClick={() => setStep(s => s - 1)} className="btn-ghost flex items-center gap-2"><ArrowLeft size={16} />Retour</button>
              : <div />}
            <button onClick={() => step === 3 ? setStep(4) : setStep(s => s + 1)} disabled={!canNext()}
              className="btn-primary flex items-center gap-2">
              {step === 3 ? "Créer mon programme" : "Continuer"}<ArrowRight size={16} />
            </button>
          </div>
        )}
      </div>
